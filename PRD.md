...
Safari는 `backdrop-filter`에 SVG 필터(`url(#container-glass)`)를 걸 수 없습니다. **그래서 Safari에서 유리 왜곡이 안 보이는 것은 버그가 아니라 예상된 동작입니다.** 헷갈리기 쉬우니, 직접 확인할 때 아래 기준으로 나눠 보세요.

**이러면 정상입니다 — 고칠 필요 없습니다**

- 버튼 뒤를 지나는 커서 궤적이 **일그러지지 않고 또렷하게** 그대로 보인다 (Chrome에서는 물결처럼 휘어 보입니다)
- 버튼이 유리처럼 뿌옇지 않고, 얇은 테두리와 안쪽 그림자로 알약 윤곽만 잡혀 있다
- 그 상태에서도 **"인터뷰 보러 가기" 글자가 또렷하게 읽힌다**
- 커서 궤적, `재탐색` 밑줄 점멸, 마퀴 흐름은 Chrome과 똑같이 동작한다 (궤적은 canvas 2D, 나머지는 일반 CSS라 엔진과 무관합니다)

**이러면 버그입니다 — 알려주세요**

| 증상 | 의심 지점 |
|---|---|
| 버튼 윤곽이 **아예 안 보인다** (배경과 구분이 안 됨) | 그림자 레이어가 죽음 |
| 글자가 배경 궤적에 묻혀 **안 읽힌다** | 라벨 `z-index` 또는 대비 |
| 버튼을 눌러도 `/interviews`로 **안 간다** | `asChild` / `Slottable` |
| 커서 궤적이 **전혀 안 그려진다** | canvas — 단, 아래 "먼저 확인" 참고 |
| 궤적이 사각형으로 잘리거나 깜빡인다 | canvas 크기·DPR 처리 |
| 마퀴 카드 클릭이 안 된다 | Safari의 3D transform 히트테스트. **치명적이진 않습니다** — 같은 링크가 하단 목록에 전부 있어 우회 가능하나, 발견하면 알려주세요 |
| 가로 스크롤이 생긴다 | 레이아웃 |
| `재탐색` 밑줄이 계속 켜져 있거나 아예 안 보인다 | `animate-blink` — 아래 "먼저 확인" 참고 |

**먼저 확인할 것** — 궤적·밑줄·마퀴가 *모두* 멈춰 있다면 버그가 아니라 **"동작 줄이기"** 설정일 수 있습니다. 의도된 동작입니다 (`§2.1`).
macOS는 `시스템 설정 → 손쉬운 사용 → 디스플레이 → 동작 줄이기`, iOS는 `설정 → 손쉬운 사용 → 동작 → 동작 줄이기`를 끄고 다시 보세요.

**지원 범위** — Safari **16.4 이상**에서 확인하세요. Next.js 16이 요구하는 최소 버전이고, 이 사이트가 쓰는 `dvh`·`:has()`·Tailwind v4 문법도 그 아래에서는 동작하지 않습니다. 16.4 미만에서 깨지는 것은 버그가 아니라 지원 범위 밖입니다.
...
- [ ] ⚠️ Safari: 왜곡이 없어도 버튼이 정상으로 읽힘, 궤적은 정상 표시 — **이 환경에 WebKit이 없어 확인하지 못했습니다.** 무엇이 정상이고 무엇이 버그인지는 `§6.4.2 Safari 판정 기준`을 보세요. 다만 (1) 왜곡과 무관한 테두리·그림자 레이어가 별도 div로 존재하고 (2) 궤적은 순수 canvas 2D라 엔진과 무관합니다. 배포 후 실제 Safari에서 한 번 봐 주세요
